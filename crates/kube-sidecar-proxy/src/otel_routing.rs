...
fn parse_traceparent(traceparent: &str) -> Option<(Option<String>, Option<String>, Option<String>)> {
...
pub fn determine_routing_target(
    ...
    if let Some(trace_id) = &routing_context.trace_context.trace_id {
        ...
        if is_high_priority_trace(trace_id) {
            ...
            return RoutingTarget::Premium { port: original_port };
        }
    }
    ...
    // Default routing
    RoutingTarget::Default { port: original_port }
}
...
mod tests {
    ...
    fn test_extract_trace_context() {
        ...
        assert_eq!(context.trace_context.trace_id, Some("4bf92f3577b34da6a3ce929d0e0e4736".to_string()));
        assert_eq!(context.trace_context.span_id, Some("00f067aa0ba902b7".to_string()));
        assert_eq!(context.trace_context.baggage.get("userId"), Some(&"alice".to_string()));
    }
