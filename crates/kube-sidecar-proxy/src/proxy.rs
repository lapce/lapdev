use crate::{
    config::{RouteConfig, RouteTarget, AccessLevel, ProxyConfig},
...
impl ProxyHandler {
    ...
    pub fn create_router(self) -> Router {
        ...
        Router::new()
            ...
            .layer(
                ServiceBuilder::new()
                    .layer(TraceLayer::new_for_http().on_request(|req: &Request<Body>, _span: &Span| {
                        debug!(
                            method = %req.method(),
                            uri = %req.uri(),
                            "Processing request"
    ...
    pub async fn proxy_request(
        ...
        if route.requires_auth {
            self.check_authorization(&headers, &route.access_level).await?;
        }
    ...
    async fn resolve_target(&self, target: &RouteTarget) -> Result<SocketAddr> {
        match target {
            ...
            RouteTarget::Service { name, namespace, port } => {
                let endpoints = self
                    .discovery
    ...
    fn prepare_headers(&self, original: &HeaderMap, route: &RouteConfig) -> Result<HeaderMap> {
        ...
        for (name, value) in &route.headers {
            let header_name = HeaderName::from_bytes(name.as_bytes())
                .map_err(|e| SidecarProxyError::Generic(anyhow::anyhow!("Invalid header name: {}", e)))?;
            let header_value = HeaderValue::from_str(value)
                .map_err(|e| SidecarProxyError::Generic(anyhow::anyhow!("Invalid header value: {}", e)))?;
...
async fn proxy_request(State(handler): State<Arc<ProxyHandler>>, req: Request) -> impl IntoResponse {
